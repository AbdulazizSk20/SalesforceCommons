...
This framework provides utility functions for Salesforce automation testing across selenium cucumber automation frameworks.
...
- **Syntax:**
  ```javascript
  async function createRecord(engine:any, object: string, record:string): Promise<Object>
  ```
  - Parameters:
...
- **Syntax:**
  ```javascript
  async function deleteAllRecord(engine:any, object: string): Promise<void>
  ```
  - Parameters:
    1. `{any} engine` - The Salesforce engine to use for deleting records.
    2. `{string} object` - The Salesforce object to delete records from.
...
- **Syntax:**
  ...
  ```
  - Parameters:
    1. `{any} driver` - Selenium instance.
